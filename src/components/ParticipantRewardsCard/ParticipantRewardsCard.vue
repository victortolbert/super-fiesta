<template>
  ...
  >
    <div :class="[ 'm-1 border-2 rounded overflow-hidden', colorClassFor(reward.status) ]">
      <header class="text-sm font-semibold">
        <div
          ...
          class="bg-red-700 text-white truncate capitalize p-4 w-full"
        >
        ...
        <div
          ...
          class="bg-orange-500 text-white truncate capitalize p-4 w-full"
        >
          {{ $t('status.pending') }}
        </div>
        <div
          ...
          :title="lang.giveaway"
          class="bg-green-600 text-white truncate capitalize p-4 w-full"
        >
          {{ $t('giveaway') }}
        </div>
        <div
          ...
          class="bg-blue-500 text-white truncate capitalize p-4 w-full"
        >
          {{ $t('delivered') }}
        </div>
      </header>
      ...
      <footer class="card-footer block bg-gray-200">
...
export default {
    ...
    reward: {
      ...
      default() {
        return {
          ...
          prizeBound: {
            display_name: 'Gobble up a Gourmet Lollipop',
            display_amount: 50,
          },
          status: 'pending',
        }
  ...
  methods: {
    ...
    getPictureUrl(reward) {
      return `https://funrun-dev.s3.amazonaws.com/prizes/${reward.prize.picture}`
